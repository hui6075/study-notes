...
static int iotx_mc_check_rule(char *iterm, iotx_mc_topic_type_t type)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (iterm[i] < 32 || iterm[i] >= 127) { /* 可读字符ASCII */
            return FAIL_RETURN;
        }
    }
...
/* Check topic name */
/* 0, topic name is valid; NOT 0, topic name is invalid */ /* 0:TOPIC_NAME_TYPE; 1:TOPIC_FILTER_TYPE */
static int iotx_mc_check_topic(const char *topicName, iotx_mc_topic_type_t type)
{
    ...
    if (NULL == topicName || '/' != topicName[0]) { /* 必须以'/'开头 */
        return FAIL_RETURN;
    }

    if (strlen(topicName) > IOTX_MC_TOPIC_NAME_MAX_LEN) { /* 最大长度128bytes */
    ...
    for (;;) {
        iterm = strtok(NULL, delim); /* split by "/" */
        ...
        if (SUCCESS_RETURN != iotx_mc_check_rule(iterm, type)) { /* 0:TOPIC_NAME_TYPE; 1:TOPIC_FILTER_TYPE */
            log_err("run iotx_check_rule error");
            return FAIL_RETURN;
        }

        if (iterm[0] == '#') { /* #出现之后就不能再有subtopic了 */
            mask = 1;
        }
    }
...
int MQTTConnect(iotx_mc_client_t *pClient)
{
    ...
    if ((len = MQTTSerialize_connect((unsigned char *)pClient->buf_send, pClient->buf_size_send, pConnectParams)) <= 0) {
        HAL_MutexUnlock(pClient->lock_write_buf); /* 序列化CONNECT消息 */
    ...
    if ((iotx_mc_send_packet(pClient, pClient->buf_send, len, &connectTimer)) != SUCCESS_RETURN) { /* 发送CONNECT */
        HAL_MutexUnlock(pClient->lock_write_buf);
                         ...
                         iotx_mqtt_event_handle_func_fpt messageHandler, void *pcontext)
{
    ...
    /* push the element to list of wait subscribe ACK */ /* 把消息放到list_sub_wait_ack里，方便重传 */
    if (SUCCESS_RETURN != iotx_mc_push_subInfo_to(c, len, msgId, SUBSCRIBE, &handler, &node)) {
        log_err("push publish into to pubInfolist failed!");
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_PUSH_TO_LIST_ERROR;
    }
    /* 发送SUBSCRIBE包，发送失败的话把SUBSCRIBE从list_sub_wait_ack中删除 */
...
static int iotx_mc_read_packet(iotx_mc_client_t *c, iotx_time_t *timer, unsigned int *packet_type)
{
    ...
    /* 1. read the header byte.  This has the packet type in it */ /* 带超时的HAL_TCP_Read */
    ...
    /* 2. read the remaining length.  This is variable in itself */ /* 读取可变length，返回可变length字节数 */
    if ((rc = iotx_mc_decode_packet(c, &rem_len, iotx_time_left(timer))) < 0) { /* rem_len是包剩余长度 */
    ...
    if ((rem_len > 0) && ((rem_len + len) > c->buf_size_read)) { /* 对于超出read buffer的mqtt消息，读取并丢弃超出部分 */
        ...
        if (NULL != c->handle_event.h_fp) { /* 通知mqtt read buffer不足事件 */
    ...
    if (rem_len > 0 && (c->ipstack->read(c->ipstack, c->buf_read + len, rem_len, iotx_time_left(timer) == 0 ? 1 : iotx_time_left(timer)) != rem_len)) {
        log_err("mqtt read error"); /* HAL_TCP_Read() */
        return FAIL_RETURN;
    }
...
static int iotx_mc_handle_recv_SUBACK(iotx_mc_client_t *c)
{
    ...
    (void)iotx_mc_mask_subInfo_from(c, mypacketid, &messagehandler); /* 根据packetid查找当初订阅的信息 */
    ...
    for (i = 0; i < IOTX_MC_SUB_NUM_MAX; ++i) { /* 查看此suback是否已记录 */
        /* If subscribe the same topic and callback function, then ignore */
        if ((NULL != c->sub_handle[i].topic_filter)) { /* 查看两个msg handler是否完全一致 */
    ...
    if (0 == flag_dup) { /* 如果没有记录，则找一个空闲位置把此suback记录下来 */
        if (-1 == i_free) {
...
#define WAIT_CONNACK_MAX (10)
    ...
    do {
        ...
        rc = iotx_mc_read_packet(c, &timer, &packetType); /* 读回一个完整的(CONNACK)包(带超时时间) */
        if (rc != SUCCESS_RETURN) {
    ...
    rc = iotx_mc_handle_recv_CONNACK(c); /* 读取session present/返回连接是否被accept */
    if (SUCCESS_RETURN != rc) {
...
static int iotx_mc_cycle(iotx_mc_client_t *c, iotx_time_t *timer)
{
    ...
    HAL_MutexLock(c->lock_generic); /* 收到任何消息，都清零keepalive计数器 */
    c->ping_mark = 0;
    c->keepalive_probes = 0;
    HAL_MutexUnlock(c->lock_generic);

    switch (packetType) { /* 根据收到的消息,构造事件event,并调用回调函数c->handle_event.h_fp(event,...) */
        case CONNACK: {
            log_debug("CONNACK");
            break;
        }
        case PUBACK: {
            log_debug("PUBACK");
            rc = iotx_mc_handle_recv_PUBACK(c); /* 标记list_pub_wait_ack, 通知 */ 
            if (SUCCESS_RETURN != rc) {
        ...
        case SUBACK: {
            log_debug("SUBACK");
            rc = iotx_mc_handle_recv_SUBACK(c); /* 把订阅信息记录下来, 调用事件回调 */
            if (SUCCESS_RETURN != rc) {
        ...
        case PUBLISH: {
            ...
            rc = iotx_mc_handle_recv_PUBLISH(c); /* 构造EVENT_PUB_RECV事件, 并调用回调函数c->handle_event.h_fp() */
            if (SUCCESS_RETURN != rc) {
        ...
        case UNSUBACK: {
            rc = iotx_mc_handle_recv_UNSUBACK(c); /* 清除client的订阅信息, 构造EVENT_UNSUB_SUCCESS并调用回调函数 */
            if (SUCCESS_RETURN != rc) {
                             ...
                             void *pcontext)
{
    ...
    if (!iotx_mc_check_state_normal(c)) { /*检查状态，只有CONNECTED状态才能订阅 */
        log_err("mqtt client state is error,state = %d", iotx_mc_get_client_state(c));
        return MQTT_STATE_ERROR;
    }

    if (0 != iotx_mc_check_topic(topicFilter, TOPIC_FILTER_TYPE)) { /* 检查主题合法性 */
    ...
    if (rc != SUCCESS_RETURN) {
        if (rc == MQTT_NETWORK_ERROR) { /* 网络出错的话设置状态为DISCONNECTED */
            iotx_mc_set_client_state(c, IOTX_MC_STATE_DISCONNECTED);
        }
...
static int iotx_mc_connect(iotx_mc_client_t *pClient)
{
    ...
    rc = pClient->ipstack->connect(pClient->ipstack); /* TCP三次握手 */
    if (SUCCESS_RETURN != rc) {
        pClient->ipstack->disconnect(pClient->ipstack);
    ...
    if (SUCCESS_RETURN != iotx_mc_wait_CONNACK(pClient)) {
		//if (iotx_mc_check_state_normal(pClient))
            (void)MQTTDisconnect(pClient); /* 如果CONNECT被拒，发送DISCONNECT(mosquitto在这个时候已经把socket关了) */
        pClient->ipstack->disconnect(pClient->ipstack); /* 关闭socket */
        log_err("wait connect ACK timeout, or receive a ACK indicating error!");
        return MQTT_CONNECT_ERROR;
    }

    iotx_mc_set_client_state(pClient, IOTX_MC_STATE_CONNECTED); /* 设置客户端状态机 */

    utils_time_countdown_ms(&pClient->next_ping_time, pClient->connect_data.keepAliveInterval * 1000);

    aos_poll_read_fd(get_iotx_fd(), cb_recv, pClient); /* 把mqtt socket加入yloop回调 */
    aos_post_delayed_action( pClient->connect_data.keepAliveInterval * 1000, cb_recv_timeout, pClient); /* 提交keepalive定时器 */
...
static int iotx_mc_disconnect(iotx_mc_client_t *pClient)
{
    ...
    if (iotx_mc_check_state_normal(pClient)) { /* 只有在CONNECTED状态才发送 */
        rc = MQTTDisconnect(pClient); /* 如果CONNECT失败，不会再次发送DISCONNECT */
...
static int iotx_mc_report_mid(iotx_mc_client_t *pclient)
{
    ...
    ret = IOT_MQTT_Publish(pclient, topic_name, &topic_info); /* 加密/带超时的阻塞发布一条PUBLISH消息 */
    if (ret < 0) {
...
{
    int                 err;
    iotx_mc_client_t   *pclient; /* 客户端上下文 */
    ...
    err = iotx_mc_init(pclient, pInitParams); /* 初始化客户端上下文 */
    ...
    if (SUCCESS_RETURN != err) { /* 连接成功的话把mqtt fd加入yloop事件循环, 把keepalive定时器加入yloop，失败的话发送Disconnect */
        iotx_mc_release(pclient); /* 失败的话又调用一次MQTTDisconnect */
        LITE_free(pclient);
        return NULL;
    }
    ...
    /* report module id */
    err = iotx_mc_report_mid(pclient); /* 上报设备信息 */
    if (SUCCESS_RETURN != err) {
...
int IOT_MQTT_Subscribe(void *handle, /* mqtt client */
...
{ /* 加密/带超时(c->request_timeout_ms)的阻塞发布一条PUBLISH消息 */
    POINTER_SANITY_CHECK(handle, NULL_VALUE_ERROR);
